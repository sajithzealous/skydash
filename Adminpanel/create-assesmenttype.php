<?php
    ...
    if($action == 'create')
    {
        ...
        $AssessmenttypeStatus = $_GET['AssessmenttypeStatus'];

        if($AssessmenttypeStatus =='Active'){
        ...
        $sql = mysqli_query($conn, "SELECT * FROM `Assessmenttype` WHERE `Assessmenttype` = '$Assessmenttype'");
        ...
        {
           $sql1 = "INSERT INTO `Assessmenttype`(`Assessmenttype`, `status`, `created_time`) VALUES ('$Assessmenttype','$AssessmenttypeStatus','$dateTime')";
    ...
    mysqli_query($conn, "UPDATE `Assessmenttype` SET `status`='$status' WHERE id = '$row_id'");

    // Prepare the response
    $res['status'] = 'Ok';

      $fromtable = 'Assessmenttype';
