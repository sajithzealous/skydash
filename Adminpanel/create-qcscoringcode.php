<?php
    ...
    if($action == 'create')
    {
        ...
        $sql = mysqli_query($conn, "SELECT * FROM `coding-scoring` WHERE `Mitem` = '$Cpsccode' AND  `Score`='$state', AND `Segement`='$weight' AND `status`=''");
    ...
    mysqli_query($conn, "UPDATE `coding-scoring` SET `status`='$status' WHERE id = '$row_id'");

    // Prepare the response
    $res['status'] = 'Ok';

     $fromtable = 'coding-scoring';
