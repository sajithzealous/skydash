      ...
      fieldData.map(
        (item, index) => `<tr id="row${
          index + 1
                ...
                <td style="padding:5px; vertical-align: middle;" class="arrowdata">
                  <div style="display: flex; flex-direction: column; align-items: center;">
                      <button type="button" class="btn move-up" data-mitem="${item['M-Items']}" style="padding: 2px 5px;">
                          <i class="fas fa-arrow-up" title="Move Up" style="color:#31029c; cursor: pointer;"></i>
                      </button>
                      <button type="button" class="btn move-down" data-mitem="${item['M-Items']}" style="padding: 2px 5px; margin-top: 5px;">
                          <i class="fas fa-arrow-down" title="Move Down" style="color:#31029c; cursor: pointer;"></i>
                      </button>
                  </div>
              </td>
        ...
        <textarea name="Qarationaile" style="color: black; font-size: 13px;" class="form-control-sm input_empty input-lg form-input Qarationaileid resizable-textarea" disabled hidden   data-mitem="${
      item["M-Items"]
   ...
   <option value="Primary"${
    ...
    $(document).ready(function() {
      ...
      $(document).on('click', '.move-up', function() {
          ...
          if (prevRow.length) {
              // Move all cells except the first four
              for (var i = 4; i < row.children('td').length; i++) {
                  var currentCell = row.children('td').eq(i);
                  var prevCell = prevRow.children('td').eq(i);
  
                  // Swap the content of the cells
                  var temp = currentCell.html();
                  currentCell.html(prevCell.html());
                  prevCell.html(temp);
              }
          }
      });
      ...
      $(document).on('click', '.move-down', function() {
          ...
          if (nextRow.length) {
              ...
              for (var i = 4; i < row.children('td').length; i++) {
                  var currentCell = row.children('td').eq(i);
                  var nextCell = nextRow.children('td').eq(i);
  
                  // Swap the content of the cells
                  var temp = currentCell.html();
                  currentCell.html(nextCell.html());
                  nextCell.html(temp);
              }
          }
      });
...
<td style="padding:1px;"><input type="checkbox" value="Coder" readonly disabled name="coder" id="flexCheckCoder" data-mitem="${randomSlug}"></td>
  ...
  $("#btn_save").on("click", function () {
    ...
    $("#dynamic_field tbody tr").each(function (index, row) {
      var icdInput = $(row).find("td:eq(4) input");
      var mitems = $(row).find("td:eq(3) input").val();
      ...
      if (icdInput.val().trim() !== "") {
        ...
        var gender = $("#gender").val();
        var EntryId = $(row).find("td:eq(2) input").val();
        var description = $(row).find("td:eq(5) input").val();
        var effectivedate = $(row).find("td:eq(6) input").val();
        var eo = $(row).find("td:eq(7) select").val();
        var rating = $(row).find("td:eq(8) select").val();
        var reason = $(row).find("td:eq(9) select").val();
        var errortype = $(row).find("td:eq(10) select").val();
        var qcrationaile = $(row).find("td:eq(11) textarea").val();
        ...
        var agencychecked = $(row)
          .find("td:eq(13) input[type='checkbox']:checked")
          .val();
        if (agencychecked === undefined) {
          agencychecked = "";
        }
        var mitemchecking = $(row).find("td:eq(14) select").val();
...
function handleAddNewCode(event) {
  ...
  $clonedRow.find(".arrowdata").hide();
  // var $emptytd = $(
  //   '<td></td>'
  // );

  // $clonedRow.append($emptytd);
  $clonedRow
...
function qcscoringdisplay() {
  ...
  $.ajax({
    type: "post",
    url: "QA/qcscoringfetch_v3.php?action=codescore",
    data: {
      Id: Id,
    },
