...
function datadisplaytable() {
  fetch("Adminpanel/qcscoringsql.php")
...
function createDataTable(data) {
  ...
  return $("#qcscoring_table").DataTable({
    lengthMenu: [
      [15, 25, 50, -1],
      [15, 25, 50, "All"],
    ],
...
function addColumnFilter(column, index) {
  ...
  ).appendTo("#qcscoring_table th.filterRow:nth-child(" + (index + 1) + ")");
...
if (this_checkbox.prop("checked")) {
  ...
  var status = "In-Active";
    ...
    $.ajax({
      url: "Adminpanel/create-qcscoring.php?action=update_status",
      type: "get",
      data: {
        row_id: row_id,
        status: status,
