<?php
    ...
    if($action == 'create')
    {
        ...
        $sql = mysqli_query($conn, "SELECT * FROM `Comorbidity_grp_high` WHERE `grp_1`='$Grpone' AND `grp_2`='$Grptwo' AND `status`='$status' AND `active_year`='$Year'");
        ...
        {
           $sql1 = "INSERT INTO `Comorbidity_grp_high`(`grp_1`, `grp_2`, `status`, `active_year`, `timestamp`) VALUES ('$Grpone','$Grptwo','$status','$Year','$dateTime')";
    ...
    mysqli_query($conn, "UPDATE `Comorbidity_grp_high` SET `status`='$status' WHERE Id = '$row_id'");

    // Prepare the response
    $res['status'] = 'Ok';


    $fromtable = 'Comorbidity_grp_high';
