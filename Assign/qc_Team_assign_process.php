...
if (isset($_POST['File_Id'])) {
    ...
    $AssignedStatus = "ASSIGNED TO QC CODER";
        ...
        if ($coderResult) {
            ...
            if (!$currentCoder) {
                $query = "INSERT INTO `$LogTable`( `entry_id`, `patient_name`, `mrn`, `phone_number`, `insurance_type`, `assesment_date`, `assesment_type`, `agency`, `status`, `Team`, `Coder`,`coder_emp_id`,`team_emp_id`) 
                    SELECT `Id`, `patient_name`, `mrn`, `phone_number`, `insurance_type`, `assesment_date`, `assesment_type`, `agency`, '$AssignedStatus', '$user','$currentcodername','$Coder_emp_id','$empid'
                    FROM `$sourceTable` 
                    WHERE `Id`='$File_Id'";
                ...
                if ($insertResult) {
                    $updateQuery = "UPDATE `$sourceTable` SET `Status` = '$AssignedStatus', `qc_person` = '$currentcodername',`qc_person_emp_id` ='$Coder_emp_id',`AssignCoder_date`= '$currentDateTime',`qc_team`='$user',`qc_team_emp_id`='$empid',`File_Status_Time`='$currentDateTime' WHERE `Id` = '$File_Id'";
