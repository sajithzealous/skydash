...
// console.log("After Formatted todate:", todate);
    ...
    // console.log("todate",todate)/srv/www/htdocs/skydash_backup/upload/agent_files

    $.ajax({
        url: "upload/agent_files/agent_count.php",
        type: "GET",
        dataType: 'json',
        data: {
            fromdate: fromdate,
        ...
        success: function (response) {
            ...
            $("#NEW").text(response.New);
            $("#processing").text(response.wip);
           
            $("#app").text(response.approved);
            $("#assing").text(response.assign);
