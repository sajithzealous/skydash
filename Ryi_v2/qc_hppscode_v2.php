...
if (isset($_POST['hppsCodeagency'], $_POST['hppsCodecoder'], $_POST['cpsccode'], $_POST['location'])) {
    ...
    $queryHPPSagency = $conn->prepare("SELECT `Weight` FROM `hpps_code` WHERE `Hipps_code`='$hppscodeagency' AND `status`='active' ");
    $queryHPPSagency->execute();
    $resultHPPSagency = $queryHPPSagency->get_result();

    // Fetch HPPS weight for coder
    $queryHPPScoder = $conn->prepare("SELECT `Weight` FROM `hpps_code` WHERE `Hipps_code`='$hppscodecoder' AND `status`='active' ");
            ...
            if ($resultHPPSCodeValue) {
                ...
                if ($result && $result->num_rows > 0) {
                    ...
                    if ($update_result) {
                        ...
                        // Check if entry exists in total_case_mix_qc table
                        $select_query_precoding = "SELECT * FROM `total_case_mix_qc` WHERE `Entry_Id`='$userId' AND `Status`='precoding'";
                        ...
                        if ($sqlupdateagency_result || $sqlinsertagency_result) {
                            ...
                            // Check if entry exists in total_case_mix_qc table
                            $select_query = "SELECT * FROM `total_case_mix_qc` WHERE `Entry_Id`='$userId' AND `Status`='postcoding'";
                            $result = $conn->query($select_query);
