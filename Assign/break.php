    ...
    if ($action == 'insertbreaktime') {
        ...
        if ($result) {
            while ($row = $result->fetch_assoc()) {
                ...
                $insert_query = "INSERT INTO `Work_Log` (`entry_id`, `patient_name`, `mrn`, `phone_number`, `insurance_type`, `assesment_date`, `assesment_type`, `agency`,`Team`,`status`, `Coder`,`break_status`,`brk_in`,`team_emp_id`,`coder_emp_id`) VALUES ('$Id', '$patient_name', '$mrn', '$phone_number', '$insurance_type', '$assessment_date', '$assessment_type', '$agency',  '$Team','$status', '$Coder','$break_status', '$currentDateTime','$Team_empid','$Coder_empid')";
