  ...
  const renderFields = (fieldData) => {
      ...
      fieldData.map(
        (item, index) => `<tr id="row${
          index + 1
            ...
            <td style="width: 10px; padding:10px;"><span > ${index + 1}</span></td>
    ...
    <input type="text" 
           ...
           title="${item["Description"]}" />
            ...
            <td style="width: 150px;padding:10px;"><input type="text" id="date-row${index + 1}" value="${
          item["Effective_Date"]
...
function manageFocus(inputs) {
    ...
    document.addEventListener('keydown', (event) => {
        if (event.key.startsWith('Arrow') && document.activeElement.tagName.toLowerCase() !== 'textarea') {
            event.preventDefault();  
    ...
    $("#dynamic_field tbody").append(
      `<tr id="row${
        rowid + 1
                    ...
                    <td style="width: 100px;padding:10px;"><span > ${serialNumber}</span></td>
