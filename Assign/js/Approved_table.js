$(document).ready(function () {
  ...
  function GetQc_Complted() {
    $.ajax({
      url: "Assign/Approved_get_process.php",
      type: "GET",
      dataType: "json",
      success: function (data) {
        ...
        if (role_user == "TeamLeader" ||  role_user == "QA" || role_user == "QaTl") {
          var table;
          ...
          if (!$.fn.DataTable.isDataTable("#Approved_table")) {
            ...
            table = $("#Approved_table").DataTable({
              ...
              columns: [
                ...
                { title: "Coder Comments" },
                { title: "Client Comments" },
                { title: "Agency Comments" },
                { title: "Chart" },
                { title: "Worked Files" },
                { title: "Apporved Files" },
              ],
            });
        ...
        if (
          ...
          if (data.length === 0) {
            ...
            data.forEach(function (item) {
              ...
              // var icon = '<td><i class="bi bi-question-circle query-icon" style="cursor: pointer; margin-bottom:25px"></i></td>';

              var comments = '<a class="btn btn-warning flow commentsreallocation" data-id="' + item.Id + '" onclick="reallocation_comments(' + item.Id + ')">Comments</a>';
...
//realloaction comments
function reallocation_comments(Id) {
  var Id = Id;
  var message = prompt(
    "Please enter your query for Selected chart   " + Id + ":"
  );
  ...
  $.ajax({
    type: "post",
    url: "Assign/reallocation_comments.php?action=reallocationcomments",
    data: {
      Id: Id,
      message: message,
    },
    ...
    success: function (response) {
      if ((response = "Reallocation comments updated")) {
        Swal.fire({
          title: "Success!",
          text: "Reallocation comments updated",
          icon: "success",
          confirmButtonText: "OK",
        }).then(function () {
          // $('.commentsreallocation[data-id="' + Id + '"]').hide();
        });
      } else {
        Swal.fire({
          title: "Error",
          text: "Reallocation comments not updated",
          icon: "error",
          confirmButtonText: "OK",
        });
      }
