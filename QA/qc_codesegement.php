...
foreach ($data as $row) {
    ...
    if ($mitems == "M1021A") {
        $primaryDiagnosisQuery = "SELECT `ICD-10-code` FROM `Codedescription` WHERE `ICD-10-code`='$icd' AND `Diagnosis_type` ='Primary'";
        ...
        if ($mitemssqlresultExist) {
            $updateQuery = "UPDATE `Codesegementqc` SET `M_Item`='$mitems', `ICD-code`='$icd', `Description`='$description', `Effective_Date`='$effectivedate', `Eo`='$eo', `Rating`='$rating' ,`Error_reason`='$reason',`Error_type`='$errortype',`Qc_rationali`='$qcrationaile',`User`='$user',`coder_emp_id` = '$emp_id'WHERE `M_Item`='$mitems' AND `Entry_id`='$Id'";
            ...
            if ($patientResult) {
                ...
                $insertQuery = "INSERT INTO `Codesegementqc`(`Entry_Id`, `Mrn`, `Patient_Name`, `Assessment_Date`, `M_Item`, `ICD-code`, `Description`, `Effective_Date`, `Eo`, `Rating`, `User`,`status`,`Error_reason`,`Error_type`,`Qc_rationali`,`coder_emp_id`) 
                                VALUES ('$Id','$mrn','$patientName','$assessmentDate','$mitems','$icd','$description','$effectivedate','$eo','$rating','$user','$status','$reason','$errortype','$qcrationaile','$emp_id')";
