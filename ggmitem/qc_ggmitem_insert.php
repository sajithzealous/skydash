 ...
 $Coderrationail = $conn->real_escape_string($Coderrationail);
        ...
        $Coder_response = json_encode($CoderData);
            if (!empty($Coderrationail)) {
                echo $Coder_rationali = $Coderrationail;
            } else if(empty($Coderrationail)) {
               
                echo $Coder_rationali = 'Null';
            }
        $status = 'Coder_review';
        ...
        if ($result && $result->num_rows > 0) {
            ...
            $emp_id = $conn->real_escape_string($row['coder_emp_id']);
                ...
                $extractedDatasql = "SELECT `M_item` FROM `oasis_qc_new` WHERE `M_item` = '$extractedData' AND `Entry_id` = '$Entry_Id'";
                $extractedDatasqlresult = $conn->query($extractedDatasql);
                $rowsExist = ($extractedDatasqlresult->num_rows > 0);

                if ($rowsExist) {
                    // Update existing record
                      $updateQuery = "UPDATE `oasis_qc_new` SET `Agency_response`='$Agency_response', `Coder_response`='$Coder_response', `Coder_rationali`='$Coder_rationali',`Error_reason`='$errorreason',`Error_type`='$errortype' ,`User`='$user', `coder_emp_id`='$emp_id' WHERE `M_item`='$extractedData' AND `Entry_id`='$Entry_Id'";
                    ...
                    if ($updateResult === TRUE) {
                        ...
                        $insertQuery = "INSERT INTO `oasis_qc_new`(`Entry_id`, `Mrn`, `Patient_Name`, `header`, `M_item`, `Agency_response`, `Coder_response`, `Coder_rationali`,`Error_reason`,`Error_type`, `User`, `coder_emp_id`, `status`) VALUES ('$Entry_Id', '$mrn', '$patientName', '$header', '$extractedData', '$Agency_response', '$Coder_response', '$Coder_rationali','$errorreason','$errortype', '$user', '$emp_id', '$status')";
