...
$query1 = "UPDATE `Codesegementpending` SET `status` = 'NOT-USED' WHERE `Entry_Id` = '$Id' AND `status`='USED'";
$query2 = "UPDATE `oasis_pending` SET `status` = 'NOT-USED' WHERE `Entry_Id` = '$Id' AND `status`='USED'";
$query3 = "UPDATE `Pocsegement_pending` SET `status` = 'NOT-USED' WHERE `Entry_Id` = '$Id' AND `status`='USED'";
...
$updateQuery = "UPDATE `Main_Data` SET `status` = 'InProgression',`File_Status_Time`= '$currentDateTime' WHERE `Id` = '$Id'";
...
$insert_query = "INSERT INTO `Work_Log` (`entry_id`, `patient_name`, `mrn`, `phone_number`, `insurance_type`, `assesment_date`, `assesment_type`, `agency`, `status`, `Team`,`Coder`,`team_emp_id`,`coder_emp_id`) VALUES ('$Id', '$patientName', '$mrn', '$phone_number', '$insurance_type', '$assessment_date', '$assessment_type', '$agency', '$status', '$Team', '$Coder','$Team_empid','$Coder_empid')";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        foreach ($data as $row) {
            ...
            if ($mitems !== '' && $icd !== '' && $description !== '' && $effectivedate !== '' && $eo !== '' && $rating !== '') {
                $sql1 = "INSERT INTO `Codesegementpending`(`Entry_Id`, `Mrn`, `Patient_Name`, `Assessment_Date`, `M-Items`, `ICD-code`, `Description`, `Effective_Date`, `Eo`, `Rating`, `User`, `status`,`coder_emp_id`) 
                VALUES ('$Id','$mrn','$patientName','$assessmentDate','$mitems','$icd','$description','$effectivedate','$eo','$rating','$user','Used','$emp_id')";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        foreach ($formDataArray as $formData) {
            ...
            if ($mitemValue !== '' && $Agency_response !== '' && $Coder_response !== '' && $Coder_rationali !== '') {
                $sql2 = "INSERT INTO `oasis_pending`(`Entry_id`, `Mrn`, `Patient_Name`, `M_item`, `Agency_response`, `Coder_response`, `Coder_rationali`, `User`, `status`,`coder_emp_id`) 
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        foreach ($formDataArraypoc as $formDatapoc) {
            ...
            if ($pocitemValue !== '' && $Coder_response !== '') {
                $sql3 = "INSERT INTO `Pocsegement_pending`(`Entry_id`, `Mrn`, `Patient_Name`, `Poc_item`, `Coder_response`, `User`, `status`,'coder_emp_id') 
