<?php
    ...
    if($action == 'create')
    {
        ...
        $Labor = $_GET['Labor'];
        $Nonlabor = $_GET['Nonlabor'];
        $Year = $_GET['Year'];
        $status = $_GET['status'];

    

        $sql = mysqli_query($conn, "SELECT * FROM `hppscodevalue` WHERE `National` ='$National' AND `Labor_Portion`='$Labor' AND `Non_Labor_Portion`='$Nonlabor' AND `Status`='$status' AND `Year`='$Year'");
        ...
        {
           $sql1 = "INSERT INTO `hppscodevalue`(`National`, `Labor_Portion`, `Non_Labor_Portion`, `Status`, `Year`, `timestamp`) VALUES ('$National','$Labor','$Nonlabor','$status','$Year','$dateTime')";
    ...
    mysqli_query($conn, "UPDATE `hppscodevalue` SET `status`='$status' WHERE Id = '$row_id'");

    // Prepare the response
    $res['status'] = 'Ok';


    $fromtable = 'hppscodevalue';
